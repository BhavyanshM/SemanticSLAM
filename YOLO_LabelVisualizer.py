...
def sync_labels():
    count = 0
    for x in facade_label_files:
        if os.path.exists(facade_img_path + x[:-4] + '.jpg'):
            print('Found: ', x[:-4] + '.jpg')
        else:
            count += 1
...
def visualize():
    for file in facade_img_files:
        img = cv2.imread(facade_img_path + file)
        labels = open(facade_label_path + file[:-4] + '.txt')
        ...
        for l in labels:
            ...
            print(x,y,w,h)
            print(classes[int(det[0])], int(det[0]))
